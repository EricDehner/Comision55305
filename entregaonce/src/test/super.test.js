...
describe("Test de E-Commerce", () => {
    ...
    describe("Test de productos.", () => {
        ...
        /* it("Obtener producto por su ID - GET /api/products/:pid", async function () {
             const productMock = generateMockProduct();
             const createProductResponse = await requester.post("/api/products").set("Cookie", globalCookie).send(productMock._id);
             expect(createProductResponse.statusCode).to.equal(200);
               console.log("Response body after creating product:", createProductResponse.body);
        ...
        /* it("Si tu rol lo permite, cargar producto estando logeado - POST /api/products/", async function () {
            const productMock = generateMockProduct();
